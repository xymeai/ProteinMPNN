...
to1letter = {aaa: a for a, aaa in zip(RES_NAMES_1, RES_NAMES, strict=False)}
to3letter = {a: aaa for a, aaa in zip(RES_NAMES_1, RES_NAMES, strict=False)}
...
idx2ra = {
    (RES_NAMES_1[i], j): (RES_NAMES[i], a)
    for i in range(20)
    for j, a in enumerate(ATOM_NAMES[i])
}

aa2idx = {
    (r, a): i
    for r, atoms in zip(RES_NAMES, ATOM_NAMES, strict=False)
...
def TMalign(chainA, chainB):
    ...
    mtx.seek(0)
    tu = np.fromstring(
        "".join(l[2:] for l in mtx.readlines()[2:5]), dtype=float, sep=" "
    ).reshape((3, 4))
    t = tu[:, 0]
    u = tu[:, 1:]
...
def parse_mmcif(filename):
    ...
    chs2num = {
        pdb2asym[ch]: r[entity_poly.getIndex("entity_id")]
    ...
    i = {
        ...
        for k, val in [
            ...
            (
                "model",
                "pdbx_PDB_model_num",
            ),  # model number (for multi-model PDBs, e.g. NMR)
        ]
    }
    ...
    for ch in chids:
        ...
        atom_seq = "".join(
            [a if m else "-" for a, m in zip(ref_seq, mask, strict=False)]
        )
        seq.append([ref_seq, atom_seq])

    metadata = {
        "method": data.getObj("exptl").getValue("method", 0).replace(" ", "_"),
        "date": data.getObj("pdbx_database_status").getValue(
            "recvd_initial_deposition_date", 0
        ),
        "resolution": res,
        "chains": chids,
        "seq": seq,
